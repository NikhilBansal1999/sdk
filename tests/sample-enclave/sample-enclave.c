...
#include "syscall.h"
#include "malloc.h"
#include "/home/nikhil/keystone/sdk/lib/verifier/sha3/sha3.h"
...
int protect_memory(void* addr, long len, int prot)
{
  int retval;
  mem memory;
  memory.addr = addr;
  memory.size = len;
  memory.prot = prot;
  ocall(6,&memory,sizeof(mem),&retval,sizeof(int));
  return retval;
}
...
void EAPP_ENTRY fibonacci_calculator()
{
  char name[] = "Nikhil Bansal\n";
  long* hash = (long*)malloc(64);
  void* result = sha3(name,14,hash,64);
  ocall_print_value(*hash);
}
